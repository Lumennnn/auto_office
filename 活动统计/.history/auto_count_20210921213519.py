...
LastEditTime: 2021-09-21 21:34:24
LastEditors: Lumen
Description:
🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍🐱‍🏍
'''
...
def activity_score(sheet : str, date, activity, root=None):
    """活动分证明统计

    Args:
        sheet (str): [description]
    ...
    for sheet_name in sheet_names:
        ...
        for x in range(data.shape[0]):
            ...
            tpl.save(root + '.\\' + the_name + '.\\' + sheet_name + '\\' + f'{person[2]}' + the_name +'.docx')

        print('--' * 20)

def second_class_score(sheet : str, date, activity, root=None):
    sheet_names = list(pd.read_excel(sheet, sheet_name=None).keys())
    print(sheet_names)
    for sheet_name in sheet_names:
        print(sheet_name)
